...
# 改修内容: 再ログイン削除 + sleep最適化 + エリアフィルタリング対応 + force_all全件取得モード追加
# ==========================================================
import sys
import os
import pandas as pd
...
if TARGET_AREA == 'force_all':
    # 全件取得時はCSVの全データを対象とし、フィルタリングを無効化
    df_active = df_map.copy()
    print("-> エリアフィルタ: 全件取得 (全エリア・フィルタリング無効)")
else:
    # 通常時のステータス除外リスト適用 ('7days_rule' を含む)
...
try:
    ...
    if collected_data:
        ...
        success_msg = f"✅ {status_prefix} {TARGET_AREA.upper()} エリアの車両データ更新が完了しました！"
        print(f"\n{success_msg}")
